- [条件とループ](#条件とループ)
  ...
  - [when](#when)
    ...
    - [when の引数を val で定義した変数で受け取る](#when-の引数を-val-で定義した変数で受け取る)
...
厳密には、 Koltin は式指向言語であり、 when もその実態は式ですが、暗黙的に Unit を返すことで、式を文的に使用しています。
...
### when の引数を val で定義した変数で受け取る

Kotlin の when や if では、その引数に val で変数を定義することができます。これは、 Kotlin の「スコープ付き変数定義」という仕組みです。 ( `if (val x = foo())` とする場合は、 else ブロック内でも x が使用できます。)
...
- `x` が `is A` などでスマートキャストされる。
- 将来、 x の取得方法に変更が発生しても、修正が一ヶ所で済む。
- 「この when ブロックでは dialog という 1つの確定した値を扱う」という意図になる。
  - つまり、 x の getter が再評価されない。
  - よって、 x の判定時 ( `is A` など) と、判定後の when ブロック内で、参照した x が同じ値であることが保証される

以下の場合は、冗長となるため、必要ありません。

- 非 nullable
- 分岐内で値を使わない
- 判定に使用する値が途中で変更される可能性がない
  - 例えば、判定値が enum である
  - 判定で `is A` などの型の判定ではなく、 `A` などのインスタンスの判定である

ただし、判断が難しい場合もあるので、 val を使用するかどうかで迷ったら、 val を使用することをお勧めします。 Kotlin の言語仕様としてもそれを推奨しています。

`when (val xxx = foo())` と書くだけなので、記述コストはほぼゼロで、たくさんのメリットが受けられるためです。
