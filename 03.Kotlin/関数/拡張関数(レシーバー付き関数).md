...
Kotlin は、クラスを継承したり Decorator などのデザインパターンを使用したりすることなく、Kotlin の新しい機能を使用すれば、クラス、または、インターフェイスを拡張することができます。これは、 **拡張関数** と呼ばれる特別な宣言を介して行われます。

たとえば、変更できないサードパーティライブラリのクラス、または、インターフェイスの新しい関数を作成できます。  
...
例えば、以下の場合は、 FileA.kt からのみアクセス可能な関数を定義できます。

```kotlin
// FileA.kt
private fun String.greetPrivate() = "Hi, $this"

fun test() {
    println("Alice".greetPrivate()) // OK、同じファイル内なら呼べる
}
```
...
// FileB.kt
fun main() {
    println("Alice".greetPrivate()) // エラー！見えない
}
```

以下の場合は、 MyClass 内からのみアクセス可能な関数を定義できます。

```kotlin
class MyClass {
    // この関数は private があってもなくても
    // どのみち外からは見えないため、意味は変わらないらしい。
    // (意味のある private の使い方のサンプルが見つかりませんでした。)
    private fun String.internalGreet() = "Hello from MyClass: $this"

    fun test() {
        println("Alice".internalGreet()) // OK
    }
}

fun main() {
    val obj = MyClass()
    obj.test() // "Hello from MyClass: Alice"
    
    // "Alice".internalGreet() // エラー、外部からは見えない
}
```
...
拡張関数は静的に解決されるため、 **コンパイル時の型情報によって、どのクラスの関数を呼び出すかが決まります。**
...
これは、 **静的ディスパッチ** や **静的バインディング** と呼ばれることもあります。

一方で、実行時にどのクラスの関数を呼び出すかが決まることを **動的ディスパッチ** や **動的バインディング** と呼びます。
...
この例で使用する `with(){}` スコープ関数については、 [スコープ関数.md - with](./スコープ関数.md/#with) を参照してください。
...
interface Density : FontScaling {
    ...
    // ただし、暗黙的に Density インスタンスを参照するため、明示的に this を記述する必要はありません。
    @Stable
...
上記の例で、スコープ関数 ( `with(){}` ) のラムダ式内は、ラッパークラスの外であるため、ラムダ式内からラッパークラスの private なメンバーにはアクセスできない点には注意してください。


## ディスパッチレシーバーと拡張レシーバー内で関数名が競合した場合

ディスパッチレシーバーと拡張レシーバー内で同じ名前の関数が存在する場合は、拡張レシーバー側の関数が優先的に参照されます。ディスパッチレシーバー側のメンバーを参照するには、 `this` とラベルを使用します。
...
同じ名前の関数が存在する場合は、どちらの関数なのかを明確にするために、常に `this@ラベル.関数呼び出し` の形式で記述することも有効な方法です。
