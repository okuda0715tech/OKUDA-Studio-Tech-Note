...
class SimpleAdMobRectangle : Activity() {
    ...
    private val mAdListener = object : AdListener() {
        override fun onAdLoaded() {
            // 広告読み込み完了時に呼ばれる。
            Toast.makeText(this@SimpleAdMobRectangle, "読み込み成功", Toast.LENGTH_SHORT).show()
        }

        override fun onAdFailedToLoad(adError: LoadAdError) {
            // 広告読み込み失敗時に呼ばれる。
            Toast.makeText(this@SimpleAdMobRectangle, "読み込み失敗", Toast.LENGTH_SHORT).show()
        }

        override fun onAdImpression() {
            super.onAdImpression()
            // 広告表示時に呼ばれる
            Toast.makeText(this@SimpleAdMobRectangle, "広告表示", Toast.LENGTH_SHORT).show()
        }

        override fun onAdClicked() {
            super.onAdClicked()
            // 広告をクリックした時に呼ばれる。
            Toast.makeText(this@SimpleAdMobRectangle, "広告クリック", Toast.LENGTH_SHORT).show()
        }
    }
