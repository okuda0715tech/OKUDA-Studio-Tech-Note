...
Composite という英単語には、 「混合体」 というニュアンスがあります。つまり、複数の素材が混ざった素材を意味します。デザインパターンにおける Composite は、「中身を表す素材」とそれを「格納する素材」が混ざったもの、あるいは混ざっている状態を意味します。

イメージを図示すると以下のようになります。

```
Component  ← 抽象型（Leaf と Composite を同一視）
├── Leaf（素材）
└── Composite（入れ物）
      ├── Leaf（素材）
      └── Composite（入れ物）
