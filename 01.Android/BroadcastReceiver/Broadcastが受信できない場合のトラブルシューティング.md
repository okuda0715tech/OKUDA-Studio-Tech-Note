...
- [Broadcast が受信できない場合のトラブルシューティング](#broadcast-が受信できない場合のトラブルシューティング)
  ...
  - [レシーバーの登録方法には動的な登録と静的な登録がある](#レシーバーの登録方法には動的な登録と静的な登録がある)
<!-- TOC END -->
...
## レシーバーの登録方法には動的な登録と静的な登録がある

静的な登録とは、マニフェストファイルにレシーバーを定義する方法です。

動的な登録とは、コードで register のような関数を呼び出して登録する方法です。

動的な登録が必要なブロードキャストインテントはいくら静的な登録をしても呼び出されません。

どのブロードキャストインテントが動的にする必要があるのかなど、その時代の Android の制約によって変わってくるため、わかりにくく、注意が必要です。
