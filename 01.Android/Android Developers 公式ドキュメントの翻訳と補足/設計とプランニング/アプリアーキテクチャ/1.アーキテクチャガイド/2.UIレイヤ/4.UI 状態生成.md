- [UI 状態生成](#ui-状態生成)
  ...
  - [状態生成パイプラインにおける入力](#状態生成パイプラインにおける入力)
    ...
    - [ストリーム API を状態変更のソースとする場合](#ストリーム-api-を状態変更のソースとする場合)
      - [combine 関数の引数は 5 個までだが](#combine-関数の引数は-5-個までだが)
...
#### combine 関数の引数は 5 個までだが

combine 関数の引数は 5 個までしか用意されておらず、そのままでは、 6 個以上の Flow を統合することはできません。そのような場合は、 combineTransform 関数を使うことで、 6 個以上の Flow を統合することができます。

combine 関数は、ラムダ式の最終行のオブジェクトを emit するようになっていますが、 combineTransform 関数は、 emit 関数を呼ぶことで、任意のタイミングで emit することが可能です。
