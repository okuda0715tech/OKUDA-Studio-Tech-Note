- [Compose における副作用](#compose-における副作用)
  ...
  - [LaunchedEffect : コンポーザブルのスコープ内で suspend 関数を実行する](#launchedeffect--コンポーザブルのスコープ内で-suspend-関数を実行する)
    ...
    - [LaunchedEffect で副作用のある処理を行う](#launchedeffect-で副作用のある処理を行う)
...
### LaunchedEffect で副作用のある処理を行う

LaunchedEffect は、キーが変更された際に 1 回だけ実行されます。つまり、再コンポーズが何度起きようと、キーが同じなら、 LaunchedEffect {} ブロック内の処理は 1 回しか実行されないということです。

この特性を生かすことで、副作用のある処理を正しく処理することが可能です。例えば、 `navController.navigate("edit")` のような、画面遷移は、再コンポーズされる度に実行されてしまうと、多重遷移する可能性があります。それを避けるために画面遷移処理を LaunchedEffect {} ブロックで囲むことが有効な対策となります。

なお、ダイアログの表示処理は、 LaunchedEffect ブロックで囲む必要はありません。なぜなら、ダイアログは、コンポーザブル関数を実行して表示される副作用のない処理だからです。何度もダイアログのコンポーザブル関数が呼ばれたとしても、ダイアログが多重表示されることはありません。
...
```kotlin
// EffectName は LaunchedEffect、produceState、DisposableEffect などのことです。
