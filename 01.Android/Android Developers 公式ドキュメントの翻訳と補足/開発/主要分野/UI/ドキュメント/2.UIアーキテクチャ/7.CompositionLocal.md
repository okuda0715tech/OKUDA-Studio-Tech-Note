...
プロパティ定義時には、初期化せず、プロパティ使用時に初期化する方法をとる場合は、以下のようにします。

```kotlin
// 型引数でプロパティの型を指定します。
val LocalElevations = compositionLocalOf<Elevations> {
    // Error オブジェクトで初期化します。
    error("Elevations not provided")
}
```

これで、プロパティを参照した際に Error オブジェクトが参照された場合は、アプリがクラッシュします。
...
ただし、この方法では、 MainActivity で LocalLogger を初期化するコードを記述する必要があり、 BaseActivity 的な役割を完全には排除できていません。この方法でも十分ですが、完全に BaseActivity 的な役割を排除する場合は、以下のようにすることができます。 (以下の例では、 LocalLogger のスコープが Activity ではなく、 Application になっていますが、趣旨を理解するには問題ありません。)
...
// 2. CompositionLocal の定義
val LocalLogger = staticCompositionLocalOf<Logger> {
    error("Logger not provided") // Provider で必ず渡すことを強制
}

// 3. Application または Activity 内の Compose エントリーポイント
@AndroidEntryPoint
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContent {
            MyApp() // Logger はここで Provider で渡す
        }
    }
}

// 4. Compose 側のトップレベル関数
@Composable
fun MyApp(
    logger: Logger = hiltLogger() // Hilt から Logger を取得
) {
    CompositionLocalProvider(LocalLogger provides logger) {
        MyScreen()
    }
...
// 5. Compose 内で Logger を使用
@Composable
fun MyScreen() {
    val logger = LocalLogger.current
    logger.log("Hello from MyScreen!")
    Text("Check logcat for Logger output")
}

// 6. Hilt で Logger を直接取得するヘルパー関数
@Composable
fun hiltLogger(): Logger {
    val context = LocalContext.current
    val entryPoint = EntryPointAccessors.fromApplication(context.applicationContext, LoggerEntryPoint::class.java)
    return entryPoint.getLogger()
}
