- [ユーザー入力を処理する](#ユーザー入力を処理する)
  ...
  - [BasicTextField をカスタマイズする](#basictextfield-をカスタマイズする)
...
## BasicTextField をカスタマイズする

BasicTextField 中身の装飾と、 BasicTextField 自身 (全体) の装飾で、その装飾方法が異なります。
...
```kotlin
    BasicTextField(
        ...
        // BasicTextField 全体に関する装飾
        modifier = Modifier
            .fillMaxWidth()
            .heightIn(min = TEXT_FIELD_MIN_HEIGHT),
        // BasicTextField の中身に関する装飾
...
上記の例では、 decorationBox のラムダには、 Box を枠として使用していますが、これは Row でも Column でもなんでも使用できます。

そして、この Box などは、 BasicTextField 自身と同じサイズになるように作られています。よって、 Box 自身にサイズを指定することはしないでください。

decorationBox のラムダ内では、 innerTextField() 以外にも Icon コンポーザブルなど、様々なものを描画することが可能です。
